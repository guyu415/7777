...
- Treat the resident Claude conversation/transcript ID as a hard continuity invariant. Unless the user explicitly asks to start a new conversation, no maintenance, tidal-memory, thinking-cleanup, deployment, recovery, or fallback path may invoke `/clear`, mint/rotate/replace the session ID, or silently switch transcript files. If a same-ID cleanup fails, stop safely and preserve the current session; changing IDs requires explicit per-incident user approval after naming the exact source and target IDs.
