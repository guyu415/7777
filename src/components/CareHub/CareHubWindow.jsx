...
function getChinaDateParts(value = new Date()) {
...
export default function CareHubWindow({ theme, onClose }) {
  ...
  const { date: todayDate, month } = getChinaDateParts()
  const monthEntries = useMemo(() => (state?.ledger.entries || []).filter((e) => e.date.startsWith(`${month}-`)), [state, month])
  const dailyEntries = useMemo(() => monthEntries.filter((entry) => entry.date === todayDate && entry.kind !== 'longTerm'), [monthEntries, todayDate])
    ...
    <div className="h-full flex flex-col" style={{
      ...
      <main className="flex-1 min-h-0 overflow-y-auto px-4 pb-4">
        ...
        {tab === 'ledger' && <div className="space-y-3">
          ...
          <Panel>
            ...
            <form onSubmit={async (e) => {
              ...
              if (await action(() => addCareLedgerEntry({ amount: expense.amount, category: selected.category, note, kind: expense.kind }))) setExpense({ ...expense, amount: '', custom: '' })
            }} className="space-y-3">
        ...
        {tab === 'settings' && draftRoles && <div className="space-y-3">
          ...
          <Panel><div className="grid grid-cols-2 gap-2"><label className="text-xs" style={{ color: '#6f8799' }}>每日目标金额<input inputMode="decimal" value={dailyBudget} onChange={(e) => setDailyBudget(e.target.value)} placeholder="0 表示不设置" className="block mt-2 w-full rounded-xl px-3 py-2.5 text-sm outline-none" style={{ background: '#f4f7f9' }} /><span className="block mt-1 text-[10px]" style={{ color: '#9aabba' }}>仅统计常规记账</span></label><label className="text-xs" style={{ color: '#6f8799' }}>每月预算<input inputMode="decimal" value={budget} onChange={(e) => setBudget(e.target.value)} placeholder="0 表示不设置" className="block mt-2 w-full rounded-xl px-3 py-2.5 text-sm outline-none" style={{ background: '#f4f7f9' }} /><span className="block mt-1 text-[10px]" style={{ color: '#9aabba' }}>包含长期开销</span></label></div></Panel>
          <button onClick={() => action(() => updateCareHubConfig({ roles: draftRoles, dailyBudget: Number(dailyBudget || 0), monthlyBudget: Number(budget || 0) }))} className="w-full rounded-2xl py-3 text-sm font-semibold text-white" style={{ background: `linear-gradient(135deg, ${primary}, #8d7bc4)` }}>保存全部设置</button>
        </div>}
      </main>
