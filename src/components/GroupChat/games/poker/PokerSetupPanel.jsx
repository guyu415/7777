...
export default function PokerSetupPanel({ theme, chat, sessions, globals, ruleNote, onStart }) {
  ...
  const start = () => {
    if (picked.length !== 2) { setError('需要正好选 2 位群成员当 AI 对手'); return }
  ...
  return (
    <main className="flex-1 overflow-y-auto px-4 py-4" style={{ minHeight: 0 }}>
      ...
      {candidates.length < 2 && <div className="text-[11px] mb-2" style={{ color: '#c9647a' }}>这个群里的成员不够 2 位，先去邀请一下吧。</div>}
      {candidates.map((c) => {
      ...
      <button
        ...
        style={{ border: 'none', borderRadius: 18, padding: 13, color: '#fff', background: `linear-gradient(135deg, ${primary}, ${primaryDark})`, fontFamily: 'inherit', fontWeight: 700, fontSize: 14.5, opacity: picked.length === 2 ? 1 : 0.5, marginBottom: 20 }}
      >
        开始这一局
      </button>
    </main>
