...
function careCompactProgress() {
  ...
  const monthSummary = ledgerMonthSummary(careHubState.ledger.entries, month)
  const monthLongTermTotal = careHubState.ledger.entries
    .filter((entry) => entry.date.startsWith(`${month}-`) && entry.kind === 'longTerm')
  ...
  return {
    ...
    spending: {
      ...
      monthLongTermTotal: Math.round(monthLongTermTotal * 100) / 100,
    },
...
function careRolePrompt(role: CareRoleId, today: string, evidence = '', solarMonth?: ReturnType<typeof baziSolarMonthContext>): { system: string; instruction: string } {
  ...
  if (role === 'ledger') {
    ...
    return {
      ...
      instruction: `这是 ${month} 的账本：月预算 ${careHubState.ledger.monthlyBudget || '未设置'} 元，每日目标 ${careHubState.ledger.dailyBudget || '未设置'} 元；今天常规支出 ${todaySummary.total} 元（${todaySummary.count} 笔），本月已记 ${summary.count} 笔，共 ${summary.total} 元；分类合计 ${JSON.stringify(summary.byCategory)}。长期开销也计入月度总额，但不计入每日目标。请做一段不超过 180 字的晚间回顾：月度进度、今日目标完成情况、最大支出类别、一个具体而温和的提醒。没有记录时就提醒用户今天可以补记。`,
    }
  }
